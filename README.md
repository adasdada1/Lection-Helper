...
`deepseek-v4-flash` с JSON Output и локальной строгой проверкой схемы. Конспект
и чек-лист создаются через OpenRouter моделями MiniMax, GLM и Nemotron.
Заголовки чатов и сжатие истории используют бесплатный пул Gemma и Laguna.
При `GROUNDING_MODE=semantic` DeepSeek получает транскрипционные фрагменты и
релевантные ИИ-конспекты, отвечает естественно своими словами и связывает
смысловые секции с transcript-источниками без обязательных дословных цитат.
Локальные проверки блокируют новые числа и идентификаторы, а DeepSeek-validator
вызывается только для смыслово рискованных секций. После фильтрации проверяется
покрытие составного вопроса; при необходимости выполняется один короткий repair
без Thinking. Простые вопросы используют `reasoning_effort=low`, составные —
`medium`. Длина выбирается адаптивно: явные просьбы «кратко» включают плотный
`brief`-ответ, обычные вопросы используют `standard`, а «подробно» — `detailed`.
Бюджет сокращает примеры и повторы, но не обязательные части ответа. Режим
`GROUNDING_MODE=strict` сохраняет прежний exact-quote pipeline.
Если основная генерация исчерпывает лимит токенов, выполняется один резервный
JSON-вызов без Thinking.
Для выбранной короткой лекции в контекст передаются все транскрипционные
фрагменты и её ИИ-конспект, если транскрипт укладывается в указанные лимиты.
